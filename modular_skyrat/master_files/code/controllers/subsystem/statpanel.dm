	...
	while(length(currentrun))
		...
		if(!target.holder)
			...
			if(target.stat_tab == "Tickets")
				...
				var/datum/interview_manager/interview_manager = GLOB.interviews

				// get open interview count
				var/dc = 0
				for (var/ckey in interview_manager.open_interviews)
					var/datum/interview/current_interview = interview_manager.open_interviews[ckey]
					if (current_interview && !current_interview.owner)
						dc++
				var/stat_string = "([interview_manager.open_interviews.len - dc] online / [dc] disconnected)"

				// Prepare each queued interview
				var/list/queued = list()
				for (var/datum/interview/current_interview in interview_manager.interview_queue)
					queued += list(list(
						"ref" = REF(current_interview),
						"status" = "\[[current_interview.pos_in_queue]\]: [current_interview.owner_ckey][!current_interview.owner ? " (DC)": ""] \[INT-[current_interview.id]\]"
					))

				var/list/data = list(
					"status" = list(
						"Active:" = "[interview_manager.open_interviews.len] [stat_string]",
						"Queued:" = "[interview_manager.interview_queue.len]",
						"Closed:" = "[interview_manager.closed_interviews.len]"),
					"interviews" = queued
				)
			...
			if(!length(GLOB.sdql2_queries) && ("SDQL2" in target.panel_tabs))
				...
				for(var/i in GLOB.sdql2_queries)
					var/datum/sdql2_query/current_query = i
					sdql2B = current_query.generate_stat()
				sdql2A += sdql2B
				target << output(url_encode(json_encode(sdql2A)), "statbrowser:update_sdql2")
		if(target.mob)
			var/mob/checked_mob = target.mob
			if((target.stat_tab in target.spell_tabs) || !length(target.spell_tabs) && (length(checked_mob.mob_spell_list) || length(checked_mob.mind?.spell_list)))
				var/list/proc_holders = checked_mob.get_proc_holders()
			...
			if(checked_mob?.listed_turf)
				var/mob/target_mob = checked_mob
				if(!target_mob.TurfAdjacent(target_mob.listed_turf))
					target << output("", "statbrowser:remove_listedturf")
					target_mob.listed_turf = null
				else if(target.stat_tab == checked_mob?.listed_turf.name || !(checked_mob?.listed_turf.name in target.panel_tabs))
